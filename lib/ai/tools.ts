...
export const portfolioTools = {
  list_projects: tool({
    description: 'List all projects in the portfolio. Optionally filter by featured status.',
  ...
  create_project: tool({
    ...
    inputSchema: z.object({
      ...
      techStack: z.array(z.string()).describe('List of technologies, e.g. ["React", "TypeScript"]'),
      imageUrl: z.string().url().optional().describe('Screenshot or preview image URL'),
      liveUrl: z.string().url().optional().describe('Live demo URL'),
      repoUrl: z.string().url().optional().describe('Source code repository URL'),
      featured: z.boolean().optional().default(false).describe('Whether to feature this project'),
      order: z.number().int().optional().default(0).describe('Display order (lower = first)'),
    }),
    execute: async (params) => executeToolCall('create_project', params),
  }),

  update_project: tool({
    description: 'Update fields of an existing project. Only provide fields to change.',
    inputSchema: z.object({
  ...
  create_provider: tool({
    ...
    inputSchema: z.object({
      ...
      baseUrl: z.string().url().optional().describe('Custom base URL (leave empty for official API)'),
      isDefault: z.boolean().optional().default(false).describe('Set as the default provider'),
    }),
