...
type ToolName =
  ...
  | 'set_featured' | 'reorder_projects'
  | 'list_providers' | 'create_provider' | 'update_provider' | 'delete_provider'
  | 'set_default_provider';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function executeToolCall(name: ToolName, params: any): Promise<unknown> {
  switch (name) {
    case 'list_projects': {
      const where = params.featured !== undefined ? { featured: params.featured } : {};
    ...
    case 'reorder_projects': {
      await Promise.all(
        params.orders.map(({ id, order }: { id: string; order: number }) =>
    ...
    case 'list_providers': {
      const providers = await prisma.aIProvider.findMany({
        ...
        select: { id: true, name: true, baseUrl: true, model: true, isDefault: true },
      });
      return { providers };
    }
